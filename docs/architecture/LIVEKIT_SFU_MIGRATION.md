...
| **1. Dual-stack behind flag** | `media_backend` per room; new rooms → LiveKit, old → P2P; SDK swap gated by flag | ✅ **DONE (2026-06-22)** — flag plumbing + SFU rooms shipped & verified. Backend: `GET /meetings/{id}/info` returns `media_backend`; `POST /meetings/{id}/livekit-token` with **role-based publishing** (host/panelist `canPublish=true`, webinar attendee `canPublish=false`); `KARAU_MEDIA_BACKEND` env + per-meeting `settings.media_backend`/`settings.is_webinar`/`settings.panelists`; default `p2p`. Frontend: `MeetingRoomSwitch` (`/room/{id}`) → `LiveKitMeetingRoom`; `WebinarLiveSwitch` (`/webinar/{id}/live`) → `LiveKitMeetingRoom webinar` (attendee view-only) — legacy P2P kept as fallback. Verified host=publish, attendee=subscribe-only (no mic/cam controls). **Remaining:** 10–20 participant load test (use `lk load-test`). |
| **1b. Load test** | 10–20 participant meeting stable | ⏳ Run LiveKit's CLI: `lk load-test --url $LIVEKIT_URL --api-key $LIVEKIT_API_KEY --api-secret $LIVEKIT_API_SECRET --room loadtest --video-publishers 5 --subscribers 15`. SFU is O(1)-upstream so this scales far beyond the old mesh; CLI confirms server capacity without 20 real browsers. |
