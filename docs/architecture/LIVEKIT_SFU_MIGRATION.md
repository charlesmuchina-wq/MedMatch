...
| **1. Dual-stack behind flag** | `media_backend` per room; new rooms → LiveKit, old → P2P; SDK swap in `MeetingRoom.jsx` gated by flag | 🟡 **STARTED (2026-06-22)** — flag plumbing + SFU room shipped & verified (host + guest). Backend: `GET /meetings/{id}/info` returns `media_backend`; `POST /meetings/{id}/livekit-token` (auth or guest); `KARAU_MEDIA_BACKEND` env + per-meeting `settings.media_backend` override; default `p2p`. Frontend: `MeetingRoomSwitch.jsx` routes `/karau-meet/room/{id}` to new `LiveKitMeetingRoom.jsx` (SFU) or the untouched P2P `MeetingRoom`. **Remaining:** webinar `/live` path (uses separate `WebinarLiveRoom`), 10–20 participant load test, recording→transcript. |
